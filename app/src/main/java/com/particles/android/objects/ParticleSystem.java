...
public class ParticleSystem {
    ...
    public void addParticle(Geometry.Point position, int color, Geometry.Vector direction, float
            particleStartTime) {
        ...
        if (nextParticle == maxParticleCount) {// Start over at the beginning, but keep
            // currentParticleCount so// that all the other particles still get drawn.
            nextParticle = 0;
        }
        ...
        particles[currentOffset++] = Color.blue
                (color) / 255f;
    ...
    public void bindData(ParticleShaderProgram particleProgram) {
        int dataOffset = 0;
        vertexArray.setVertexAttribPointer(dataOffset, particleProgram
        ...
        vertexArray.setVertexAttribPointer(dataOffset, particleProgram
                .getDirectionVectorAttributeLocation(), VECTOR_COMPONENT_COUNT, STRIDE);
        dataOffset += VECTOR_COMPONENT_COUNT;
        vertexArray.setVertexAttribPointer(dataOffset, particleProgram
                .getParticleStartTimeAttributeLocation(), PARTICLE_START_TIME_COMPONENT_COUNT,
                STRIDE);
    }
